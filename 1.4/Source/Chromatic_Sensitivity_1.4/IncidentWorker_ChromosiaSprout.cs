...
namespace Chromatic_Sensitivity
{
  ...
  {
    private static readonly IntRange CountRange = new IntRange(3, 7);
    ...
    protected override bool TryExecuteWorker(IncidentParms parms)
    {
      ...
      for (var index = 0; index < randomInRange && CellFinder.TryRandomClosewalkCellNear(cell, map, SpawnRadius, out var result, x => CanSpawnAt(x, map)); ++index)
      {
    ...
    private bool TryFindRootCell(Map map, out IntVec3 cell) => CellFinderLoose.TryFindRandomNotEdgeCellWith(10, x => CanSpawnAt(x, map) && x.GetRoom(map).CellCount >= MinRoomCells, map, out cell);

    private bool CanSpawnAt(IntVec3 c, Map map)
    {
      if (!c.Standable(map) || c.Fogged(map) || map.fertilityGrid.FertilityAt(c) < (double) ChromaticDefOf.Plant_Taggerung_Chromosia.plant.fertilityMin || !c.GetRoom(map).PsychologicallyOutdoors || c.GetEdifice(map) != null || !PlantUtility.GrowthSeasonNow(c, map))
        return false;
