...
internal class HarmonyPatch_PawnGraphicSet
{
    ...
    [HarmonyPostfix]
    private static void Postfix(ref Graphic __result, Pawn pawn, PawnRenderNode __instance)
    {
        if (
            __result == null
            ...
            || (hediff.SkinColor == null && hediff.HairColor == null)
        )
        {
            return;
        }
        ...
        if (
            hediff.SkinColor.HasValue
            && __instance.Props.colorType != PawnRenderNodeProperties.AttachmentColorType.Hair
            && !__result.Color.IndistinguishableFrom(hediff.SkinColor.Value)
        )
        {
        ...
        if (
            !hediff.HairColor.HasValue
            || __instance.Props.colorType != PawnRenderNodeProperties.AttachmentColorType.Hair
            || __result.Color.IndistinguishableFrom(hediff.HairColor.Value)
        )
        {
            return;
        }
