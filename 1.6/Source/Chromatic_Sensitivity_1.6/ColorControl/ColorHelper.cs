...
public class ColorHelper
{
    ...
    public Color? ExtractDominantColor(ThingDef thingDef)
    {
        return GetDefColorOverride(thingDef.defName)
            ?? (
                thingDef.comps.Find(c => c.compClass == typeof(CompChromaticFood)) is CompProperties_ChromaticFood comp && comp.GetForcedColor().HasValue
                    ? comp.forcedColor
    ...
    {
        return ChromaticSensitivity.Settings.ThingDefColors.TryGetValue(defName, out Color colorForDef) ? colorForDef : null;
    }

    public Color? ExtractDominantColor(Thing thing)
    {
        return GetDefColorOverride(thing.def.defName)
            ?? thing.TryGetComp<CompChromaticFood>()?.Props?.GetForcedColor()
            ?? ExtractDominantColor((Texture2D)thing.Graphic.MatSingle.mainTexture);
    }

    public Color? ExtractDominantColor(Texture2D texture)
    {
        return texture == null || texture == BaseContent.BadTex ? null : ExtractBestColor(texture.isReadable ? texture : TextureAtlasHelper.MakeReadableTextureInstance(texture));
    }
    ...
    public static Color UnpackColor(int compactedColor)
    {
        return UnpackColor32(compactedColor);
    }

    public static Color32 UnpackColor32(int compactedColor)
    {
        return new Color32((byte)(255 & compactedColor), (byte)(((255 << 8) & compactedColor) >> 8), (byte)(((255 << 16) & compactedColor) >> 16), byte.MaxValue);
    }
    ...
    private static Color? ExtractBestColor(Texture2D texture2D)
    {
        ...
        foreach (
            IGrouping<int, Color32> countedColor in texture2D
                .GetPixels32()
                .Where(p => p.a > 5) // Ignore anything that's basically transparent
                .GroupBy(CompactColor)
        )
        {
            int newCommonality = countedColor.Count();
            if (newCommonality <= commonality && !ChromaticSensitivity.Settings.ExcludedColors.ContainsKey(bestKey))
            {
                continue;
            }
        ...
        return bestColor is { } chosen ? new Color32(chosen.r, chosen.g, chosen.b, byte.MaxValue) : null;
    }
}
