...
internal class CompoundColorManager : IColorManager
{
    ...
    {
        return _subManagers.Select(manager => manager.GetSkinColor(pawn)).Where(color => color.HasValue).FirstOrFallback();
    }
    ...
    {
        return _subManagers.Select(manager => manager.GetHairColor(pawn)).Where(color => color.HasValue).FirstOrFallback();
    }
