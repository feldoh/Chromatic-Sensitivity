...
public class Hediff_ChromaticSensitivity : HediffWithComps
{
    ...
    [UsedImplicitly]
    public Hediff_ChromaticSensitivity()
        : this(null, null) { }
    ...
    public override void PostRemoved()
    {
        ...
        Log.Verbose($"Restoring pawn base color to {restoredSkinColor} (Original: {OriginalColor}) from {ChromaticSensitivity.ColorManager.GetSkinColor(pawn)}");
        ChromaticSensitivity.ColorManager.SetSkinColor(pawn, restoredSkinColor);

        if (OriginalHairColor is { } restoredHairColor)
        {
            Log.Verbose($"Restoring pawn base color to {restoredHairColor} (Original: {OriginalHairColor}) from {ChromaticSensitivity.ColorManager.GetSkinColor(pawn)}");
    ...
    {
        base.Tick();
        if (!pawn.Spawned || !pawn.IsHashIntervalTick(GenTicks.TickLongInterval) || Rand.Chance(0.2f) || !ChromaticSensitivity.Settings.AnyPeriodicEffect())
        {
            return;
        }
    ...
    {
        if (target.GetColor(ChromaticSensitivity.ColorManager, pawn) is not { } currentColor)
        {
            return;
        }
        ...
        {
            MaybeGainFavouriteSurroundingColorThought(dominantSurroundingColor.Value);
        }

        switch (target.PeriodicChromaticColorType())
        {
            case ChromaticColorType.Dominant when dominantSurroundingColor.HasValue:
                target.SetColor(ChromaticSensitivity.ColorManager, pawn, MoveColorsCloser(currentColor, dominantSurroundingColor.Value, ChromaticSensitivity.Settings.Severity));
                break;
 ...
 */
    public void ApplySurroundingEffect(Color? dominantColor)
    {
        ...
        Color.RGBToHSV(safeDominantColor, out float hue, out float saturation, out float lightness);
        if (
            (hue < 0.013 || (hue < 0.041 && saturation > 0.85) || hue > 0.941)
            && safeDominantColor.g < 0.27
            && safeDominantColor.b < 0.27
            && safeDominantColor.r > safeDominantColor.b + safeDominantColor.g
        )
        {
    ...
    public Color? DominantSurroundingColor(IntVec3 rootCell, Map map)
    {
        ...
        bool listColorOrigin = ChromaticSensitivity.Settings.VerboseLogging;
        ...
        void RecordThingColor(Color color, string name)
        {
            ...
            if (!thingsOfColor.ContainsKey(color))
            {
                thingsOfColor.SetOrAdd(color, new HashSet<string>());
            }
        ...
        {
            if (color is { r: 1.0f, g: 1.0f, b: 1.0f } or { r: 0.0f, g: 0.0f, b: 0.0f })
            {
                return false;
            }

            int colorCount = surroundingColors.TryGetValue(color) + increment;
        ...
        for (int cellIndex = 0; cellIndex < num; ++cellIndex)
        {
            IntVec3 intVec3 = rootCell + GenRadial.RadialPattern[cellIndex];
            if (!intVec3.InBounds(map) || intVec3.Fogged(map) || !GenSight.LineOfSight(rootCell, intVec3, map))
            {
                continue;
            }
            ...
            foreach (Thing thing in intVec3.GetThingList(map))
            {
                ...
                else if (thing is Plant { LeaflessNow: false } && _colorHelper.ExtractDominantColor(thing) is { } plantColor && UpdateColorCommonality(plantColor))
                {
                    RecordThingColor(plantColor, thing.def.defName);
                }

                if (thing is not Building { DrawColor: var drawColor, def: var thingDef })
                {
                    continue;
                }
                ...
                if (!UpdateColorCommonality(drawColor))
                {
                    continue;
                }
            ...
            if (!UpdateColorCommonality(floorColor))
            {
                continue;
            }
    ...
    private Func<Color, ThingDef, Color> ColorModifierFromThingDefWithAmount(float amount)
    {
        return (color, ingredient) => MoveColorTowardsIngredientColor(color, ingredient, amount);
    }
    ...
    public bool ApplyColor(ColorChangeTarget target, Color startingColor, Color newColor, bool applyThought)
    {
        ...
        switch (target)
        {
            case ColorChangeTarget.Skin:
                ...
                {
                    MoteMaker.ThrowText(pawn.DrawPos, pawn.Map, "TextMote_ChromaticSensitivity_FeelingBlue".Translate(), 6.5f);
                }
    ...
    public void FoodIngested(Thing food, CompProperties_ChromaticFood compProperties)
    {
        LongEventHandler.ExecuteWhenFinished(() =>
        {
            ...
            if (!ChromaticSensitivity.Settings.AnyIngestionEffect())
            {
                return;
            }

            float chromaticIntensity = Mathf.Clamp01(compProperties.chromaticIntensity * Severity);
            Color? forcedColor = ChromaticSensitivity.Settings.ThingDefColors.TryGetValue(defName, out Color defForcedColor) ? defForcedColor : compProperties.GetForcedColor();

            bool gainedThought = ApplyIngestionColorChange(ColorChangeTarget.Skin, false, forcedColor, compProperties.chromaticColorType, chromaticIntensity, food);
    ...
    public bool ApplyIngestionColorChange(
        ColorChangeTarget target,
        bool alreadyGainedThought,
        Color? forcedColor,
        ChromaticColorType compEffect,
    ...
    )
    {
        ...
        if (globalSetting == ChromaticColorType.None || target.GetColor(ChromaticSensitivity.ColorManager, pawn) is not { } startingColor)
        {
    ...
    private bool MaybeGainChromaticFoodThought(Color color)
    {
        ...
        pawn.needs.mood?.thoughts?.memories?.TryGainMemory(
            ColorIsSimilarToFavourite(color) ? ChromaticDefOf.Taggerung_AteExcitingChromaticFoodChromavore : ChromaticDefOf.Taggerung_AteFoodChromavore
        );
        return true;
    }
    ...
    {
        if (!ColorIsSimilarToFavourite(color))
        {
            return false;
        }
    ...
    {
        if (map is null || thoughtDef is null)
        {
            return;
        }
        ...
        LongEventHandler.ExecuteWhenFinished(() =>
        {
            Color? maybeDominantSurroundingColor = DominantSurroundingColor(originPoint, map);
            if (maybeDominantSurroundingColor is not { } dominantSurroundingColor || !ColorIsSimilarToFavourite(dominantSurroundingColor))
            {
                return;
            }
    ...
    {
        if (!(ColorIsSimilarToFavourite(thing.DrawColor) || ColorIsSimilarToFavourite(thing.DrawColorTwo)))
        {
            return;
        }
 ...
 */
    ...
    private bool ColorIsSimilarToFavourite(Color color)
    {
        return pawn?.story?.favoriteColor is { } favoriteColor
            && Mathf.Abs(color.r - favoriteColor.color.r) + Mathf.Abs(color.g - favoriteColor.color.g) + Mathf.Abs(color.b - favoriteColor.color.b)
                < favoriteColourSimilarityLeeway;
    }
}
